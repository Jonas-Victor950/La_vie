...
class PsicologoController {
    static async allPsicologos (req: Request, res: Response) {
        ...
        try{
            ...
            if (psicologos.length <= 0) {
                ...
                return res
                    .status( 200)
                    .json({success: false, msg: MESSAGE.ERROR.PSICOLOGOS.NONE_PSICOLOGO_UNTIL_NOW});
            } else {
                Logger.info(MESSAGE.SUCCESS.PSICOLOGOS.PSICOLOGOS_FIND);
                return res
                    .status(200)
                    .json({success: true, msg: MESSAGE.SUCCESS.PSICOLOGOS.PSICOLOGOS_FIND, data: psicologos});
            };
        } catch (error: any) {
    ...
    static async getOne (req: Request, res: Response) {
        try {
            ...
            if (!psicologo) {
                ...
                return res
                .status(500)
                .json({ success: false, msg: MESSAGE.ERROR.PSICOLOGOS.PSICOLOGO_NOT_FOUND });
            } else {
    ...
    static async create (req: Request, res: Response) {
        ...
        try {
            ...
            return res
                .status(200)
                .json({ success: true, msg: MESSAGE.SUCCESS.PSICOLOGOS.PSICOLOGOS_CREATE, data: psicologo });
    ...
    static async update (req: Request, res: Response) {
        ...
        try {
            ...
            if (!psicologo) {
                ...
                const updatedPsicologo = await PsicologoService.updatePsicologo(psicologoId, psicologoObj);

                Logger.info(MESSAGE.SUCCESS.PSICOLOGOS.PSICOLOGOS_UPDATE);
                return res.status(200).json({ success: true, msg: MESSAGE.SUCCESS.PSICOLOGOS.PSICOLOGOS_UPDATE, data: psicologoObj });
            }
        } catch (error) {
            Logger.error(error);
            return res
    ...
    static async delete (req: Request, res: Response) {
        try {
            ...
            return res.status(200).json({ success: true, msg: MESSAGE.SUCCESS.PSICOLOGOS.PSICOLOGOS_DELETE });
            }
        } catch (error) {
            Logger.error(error);
            return res
